...
/**
 * This class uses the <a href="https://mediaarea.net/en/MediaInfo">MediaInfo</a> library to gain information of video files.
 * <br>
 * This code was copied and modified from the
 * <a href="https://mediaarea.net/download/binary/libmediainfo0/20.03/MediaInfo_DLL_20.03_Windows_x64_WithoutInstaller.zip">source</a>.
 * This class can be found in <code>Developers/Source/MediaInfoDLL/MediaInfoDLL.JNA.java</code>.
 */
public class MediaInfo {
    private static final String LIB_WIN_FILE = "MediaInfo.dll";
    private static String LibraryPath = discover();
    ...
    static {
        ...
        try {
            ...
            if (os != null && !os.toLowerCase().startsWith("windows") && !os.toLowerCase().startsWith("mac")) {
                ...
                if (loader != null) {
                    LocalPath = Objects.requireNonNull(loader.getResource(MediaInfo.class.getName().replace('.', '/') + ".class"))
                            .getPath().replace("MediaInfo.class", "");
                    try {
                ...
                if (LocalPath.length() > 0) {
                    try {
                        ...
                        throw new UnsatisfiedLinkError("Unable to load library 'libmediainfo'");
                    }
                }
            }
        } catch (LinkageError e) {
            throw new UnsatisfiedLinkError("Failed to preload libzen");
        }
    }

    private static String discover() {
        String path = System.getProperty("mediainfo.library.path");
        System.out.print("mediainfo.library.path = ");
        System.out.println(path);
        if (path != null)
            return path + File.separatorChar + LIB_WIN_FILE;

        // Standard path: lib/mediainfo in the same folder as jar
        File f = new File(MediaInfo.class.getProtectionDomain().getCodeSource().getLocation().getPath()).getParentFile();
        return URLDecoder.decode(f.getPath(), StandardCharsets.UTF_8)
                + File.separatorChar + "lib"
                + File.separatorChar + "mediainfo"
                + File.separatorChar + LIB_WIN_FILE;
    }

    // Internal stuff
    interface MediaInfoDLL_Internal extends Library {

        MediaInfoDLL_Internal INSTANCE = Native.load(LibraryPath, MediaInfoDLL_Internal.class,
...
//    }
    ...
    public static void main(String[] args) {
        /*
        Example usage.
        1) Set path to DLL in mediainfo.library.path, e.g. -Dmediainfo.library.path=lib/mediainfo
        2) Replace videoPath with real video file path
        3) Run this class
         */

        String videoPath = "/path/to/example/video";
        String To_Display = "";

        MediaInfo MI = new MediaInfo();

        To_Display += "\r\n\r\nOpen\r\n";
        if (MI.Open(videoPath) > 0)
            To_Display += "is OK\r\n";
        else
        ...
        System.out.println("FPS:           " + MI.Get(MediaInfo.StreamKind.Video, 0, "FrameRate", MediaInfo.InfoKind.Text));
        System.out.println("Duration (ms): " + MI.Get(MediaInfo.StreamKind.Video, 0, "Duration", MediaInfo.InfoKind.Text));
        System.out.println("Width:         " + MI.Get(MediaInfo.StreamKind.Video, 0, "Width", MediaInfo.InfoKind.Text));
        System.out.println("Height:        " + MI.Get(MediaInfo.StreamKind.Video, 0, "Height", MediaInfo.InfoKind.Text));
